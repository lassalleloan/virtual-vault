...
namespace VirtualVault.Database
{
    public partial class VaultDatabase
    {
        ...
        public static bool DeleteAuthenCard(string _authenCardID)
        {
            try
            {
                ...
                using (SqlConnection connectionSQL = new SqlConnection(Data_VaultDatabase.Default.VaultDatabaseConnectionString))
                {
                    ...
                    {
                        const int NUMBER_LINE_ASSIGNED = 1;
                        ...
                        success = (commandSQL.ExecuteNonQuery() == NUMBER_LINE_ASSIGNED);
                    }
                }
        ...
        /// <summary>
        /// Obtient le statut du favoris d'une fiche d'authentification.
        /// </summary>
        /// <param name="_authenCardID">Identifiant de fiche d'authentification</param>
        /// <param name="_categoryName">Nom d'une catégorie</param>
        /// <param name="_bookmark">Statut du favoris</param>
        /// <returns>Boolean : True -> Contenu des notes sécurisées d'une fiche d'authentification. False -> .</returns>
        public static bool GetACardBookmark(string _authenCardID = "", string _categoryName = "")
        {
            try
            {
                using (SqlConnection connectionSQL = new SqlConnection(Data_VaultDatabase.Default.VaultDatabaseConnectionString))
        ...
        /// Exception levée par l'objet SqlConnection, SqlCommand ou SqlDataReader.
        /// </exception>
        public static List<string> GetACardIDList(string _authenCardID = "", string _categoryName = "", string _bookmark = "")
        {
            try
            {
                using (SqlConnection connectionSQL = new SqlConnection(Data_VaultDatabase.Default.VaultDatabaseConnectionString))
                        ...
                        using (SqlDataReader dataReader = commandSQL.ExecuteReader())
                        {
                            ...
                            if (dataReader.HasRows)
                            {
                                ...
                                while (dataReader.Read())
                                {
                                    ...
                                    aCardsNameList.Add(dataReader["idAuthenCard"].ToString());
                                }
        ...
        public static bool UpdateAuthenCardData(List<string> _authenCardData)
        {
            try
            {
                ...
                using (SqlConnection connectionSQL = new SqlConnection(Data_VaultDatabase.Default.VaultDatabaseConnectionString))
                {
                    ...
                    string queryUpdate_3 = "[authenCardBookmark]=@authenCardBookmark, [authenCardChangeDate]=@authenCardChangeDate, [idCategory]=@idCategory WHERE idVault=@idVault AND idAuthenCard=@idAuthenCard;";
